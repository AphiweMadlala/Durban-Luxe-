...
<section class="home-hero">
  <div class="container home-hero__grid">
    <div class="home-hero__copy reveal">
      <h1 class="display">Holiday homes on the Durban coast</h1>
    ...
    <figure class="home-hero__media">
      <a href="${base}stays/${heroP.slug}/" class="home-hero__imglink">${img(base, heroImg, { alt: `Pool deck facing the ocean at ${heroP.name}`, sizes: '(min-width: 1024px) 56vw, 100vw', eager: true })}</a>
...
<section class="section signature" aria-labelledby="sig-title">
  <div class="container">
    <h2 id="sig-title" class="h2 reveal">Signature stays</h2>
    <div class="signature__grid">
      <a class="signature__lead reveal" href="${base}stays/${lead.slug}/">
        <div class="signature__media">${img(base, heroImage(lead), { alt: `${lead.name}, ${placeName(lead)}`, sizes: '(min-width: 1024px) 50vw, 100vw' })}</div>
        <div class="signature__text">
          <p class="card__place">${esc(placeName(lead))}</p>
          <h3 class="h3">${esc(lead.name)}</h3>
          <p>${esc(lead.summary)}</p>
          ${specs(lead)}
          <p class="card__price">${priceLine(lead, { short: true })}</p>
        </div>
      </a>
      ${rest.map((p) => `<div class="signature__item reveal">${card(base, p, { sizes: '(min-width: 1024px) 23vw, (min-width: 640px) 45vw, 92vw' })}</div>`).join('')}
    </div>
  </div>
</section>
...
<section class="section coasts" aria-labelledby="coasts-title">
  <div class="container">
    <h2 id="coasts-title" class="h2 reveal">Where you'll stay</h2>
    <div class="coasts__grid">
      ${regions.map((r) => `<div class="coast reveal">
        <a class="coast__media" href="${base}stays/?region=${r.id}" tabindex="-1" aria-hidden="true">${img(base, r.image, { alt: '', sizes: '(min-width: 768px) 45vw, 100vw' })}</a>
        <h3 class="h3"><a href="${base}stays/?region=${r.id}">${esc(r.label)}</a> <span class="coast__count">${r.count} stays</span></h3>
        <p>${esc(r.blurb)}</p>
        <ul class="coast__areas" role="list">${r.areas.map((a) => `<li><a href="${base}stays/?area=${a.id}">${esc(a.label)} <span>${a.count}</span></a></li>`).join('')}</ul>
      </div>`).join('')}
    </div>
  </div>
</section>
...
<section class="section featured" aria-labelledby="featured-title">
  <div class="container featured__grid">
    <div class="featured__main reveal">${img(base, byIdx(featured, 17), { alt: `Timber pool deck with ocean view at ${featured.name}`, sizes: '(min-width: 1024px) 58vw, 100vw' })}</div>
    <div class="featured__side">
      <div class="featured__text reveal">
        <h2 id="featured-title" class="h2">${esc(featured.name)}</h2>
        <p>${esc(featured.description[0])}</p>
        ${specs(featured, { withIcons: true })}
        <p class="card__price">${priceLine(featured)}</p>
        <a class="btn btn--ghost" href="${base}stays/${featured.slug}/">View this stay</a>
      </div>
      <div class="featured__pair reveal">
        ${img(base, byIdx(featured, 3), { alt: `Living room at ${featured.name}`, sizes: '(min-width: 1024px) 19vw, 50vw' })}
        ${img(base, byIdx(featured, 16), { alt: `Freestanding bath by the window at ${featured.name}`, sizes: '(min-width: 1024px) 19vw, 50vw' })}
      </div>
    </div>
  </div>
</section>
...
<section class="section preview" aria-labelledby="preview-title">
  ...
  <div class="rail" tabindex="0" role="region" aria-label="More stays, scroll sideways">
    <ul class="rail__track" role="list">
      ${preview.map((p) => `<li class="rail__item">${card(base, p, { sizes: '(min-width: 1024px) 22vw, 70vw' })}</li>`).join('')}
    </ul>
  </div>
</section>
...
<section class="section views" aria-labelledby="views-title">
  <div class="container">
    <div class="views__head reveal">
      <h2 id="views-title" class="h2">The view from here</h2>
      <p>Straight from the homes, as posted on <a class="text-link" href="${business.instagram.url}" rel="noopener" target="_blank">${esc(business.instagram.handle)}</a>.</p>
    </div>
    <ul class="views__grid" role="list">
      ${views.map(({ p, im }) => `<li class="reveal"><a href="${base}stays/${p.slug}/">${img(base, im, { alt: `View from ${p.name}`, sizes: '(min-width: 1024px) 30vw, 50vw' })}<span class="views__cap">${esc(p.name)}</span></a></li>`).join('')}
    </ul>
  </div>
</section>
...
<section class="section cta" aria-labelledby="cta-title">
  <div class="container cta__grid">
    ...
    <div class="reveal">${enquiryForm(ctx, { properties: P, id: 'home-enquiry' })}</div>
  </div>
</section>`;
  ...
  return layout(ctx, {
    ...
    preload: `<link rel="preload" as="image" href="${base}${heroImg.src}-1024.webp" imagesrcset="${[640, 1024, 1440].map((w) => `${base}${heroImg.src}-${w}.webp ${w}w`).join(', ')}" imagesizes="(min-width: 1024px) 56vw, 100vw" fetchpriority="high">`,
    jsonLd: [{
